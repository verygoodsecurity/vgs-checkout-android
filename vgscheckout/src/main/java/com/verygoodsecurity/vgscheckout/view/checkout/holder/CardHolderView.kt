...
internal class CardHolderView @JvmOverloads internal constructor(
    ...
    private val cardHolderViewHolder = InputFieldViewHolder<VGSEditText>(
    ...
    override fun applyConfig(config: CheckoutFormConfiguration) {
        cardHolderViewHolder.input.setFieldName(config.cardOptions.cardHolderOptions.fieldName)
    }

    override fun getInputViews(): Array<InputFieldView> = arrayOf(cardHolderViewHolder.input)
    ...
    private fun updateErrorMessage(state: InputFieldViewHolder.ViewState) {
        val (message, drawable) = when {
            isInputEmpty(state) -> getString(R.string.vgs_checkout_card_holder_empty_error) to errorDrawable
            else -> null to null
        }
